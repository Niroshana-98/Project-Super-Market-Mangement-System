        ...
        private void btnEnter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        connection.Open();
                        command=new SqlCommand("UPDATE tbProduct SET Qty = Qty - " + int.Parse(cashier.dgvCash.Rows[i].Cells[5].Value.ToString()) + "WHERE Pcode='" + cashier.dgvCash.Rows[i].Cells[2].Value.ToString() + "'",connection);
                        command.ExecuteNonQuery();
                        connection.Close();

                        connection.Open();
                        command = new SqlCommand("UPDATE tbCart SET Status = 'Sold' WHERE Id='" + cashier.dgvCash.Rows[i].Cells[1].Value.ToString() + "'", connection);
                        command.ExecuteNonQuery();
                        connection.Close();
                    }
